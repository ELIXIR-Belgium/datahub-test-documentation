---
title: ENA Export in DataHub
sidebar: use-cases
permalink: /use-cases/ena-export
github:
    repository_url: https://github.com/ELIXIR-Belgium/DataHub-help-usecase
...
1. Under Other Projects, select the "ENA submission example" Project to display only the 4 ENA export related Sample Types.
   ...
   ![datahub ENA sample types]({{"/images/usecases-dh/ena-export-sample-types.png" | relative_url}}){:.screenshot}

2. Click the Sample Type you wish to use and click the 'create sample' button.

   ![datahub Create ENA samples]({{"/images/usecases-dh/ena-export-create-sample-button.png" | relative_url}}){:.screenshot}

3. Add the information requested in each attribute field presented. An explanation of each attribute for the 4 Sample Types is provided at the end of this page.

   ![datahub ENA_study sample example]({{"/images/usecases-dh/ena-export-create-sample-form.png" | relative_url}}){:.screenshot}

4. After adding all the required sample information, return to the sample type page and click the 'view samples' button. This will show all samples in a table.

    ![Samples table]({{"/images/usecases-dh/ena-export-view-samples-button.png" | relative_url}}){:.screenshot}

5. Click the 'Export table' button to export the samples to a csv file.

   ![datahub ENA export]({{ "/images/usecases-dh/ena-export-samples-export-button.png" | relative_url }}){:.screenshot}

6. Unfortunately, the ENA upload CLI does not ingest CSV (comma separated values) files but TSV (tab separated values) files. This requires you to save the exported CSV file as a TSV file.

7. Repeat steps 2-6 for the other sample types.

8. The resulting tables are ready for submission to ENA using the [ENA-upload-cli](https://github.com/usegalaxy-eu/ena-upload-cli) tool.
